...
uint32_t calculate_checksum(std::istream& file)
{
	...
	try
	{
		...
		//Sum is safe here
		uint32_t pe_checksum_pos = header.e_lfanew + sizeof(image_file_header) + sizeof(uint32_t) + checksum_pos_in_optional_headers;
		...
		checksum += static_cast<uint64_t>(filesize);
	}
